...
public class BeanInscripcion implements Serializable {
			...
			try {
				...
				if (matricula.getId() != 0) {
					...
					FacesContext.getCurrentInstance().addMessage(null, 
							new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR AL OBTENER LOS DATOS DE LA MATRICULA.", null));
				}
			} catch (Exception e) {
				e.printStackTrace();
				FacesContext.getCurrentInstance().addMessage(null, 
						new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR AL OBTENER LOS DATOS DE LA MATRICULA. Error: " + e.getMessage(), null));
			}						
		}
	}
		...
		try {
			...
			if (validaPago) {
				...
				if (updtInscri != 0 && updtMatrAlumno != 0 && actCuota) {
					...
					msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR EN LA BAJA DE LA INSCRIPCION, "
							+ "INT�NTELO NUEVAMENTE", null);
				}
			} else {
				msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "LA INSCRIPCI�N POSEE PAGOS! NO ES POSIBLE DAR DE BAJA! "
						+ "CONT�CTESE CON EL ADMINISTRADOR", null);
			}			
			FacesContext.getCurrentInstance().addMessage(null, msg);
		} catch (Exception e) {
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR AL REGISTRAR LA BAJA DE LA INSCRIPCION! "
	...
	public void alta(Inscripcione inscri) {
		...
		if (idInscri != 0) {
			...
			msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR EN EL ALTA DE LA INSCRIPCION, "
					+ "INT�NTELO NUEVAMENTE", null);
		}
		FacesContext.getCurrentInstance().addMessage(null, msg);
	}
...
//						String priVencimiento = formato.format(primerVencimiento);
				...
				if (existeEmail) {
					msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "YA EXISTE USUARIO CON ESE CORREO ELECTR�NICO, POR FAVOR COLOQUE UNO DISTINTO!", null);
					FacesContext.getCurrentInstance().addMessage(null, msg);
				} else {
					msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "LOS PARAMETROS" + mensaje + " SON OBLIGATORIOS PARA UNA INSCRIPCI�N", null);
					FacesContext.getCurrentInstance().addMessage(null, msg);
				}				
			}		
			return retorno;
