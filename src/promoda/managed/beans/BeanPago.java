...
public class BeanPago implements Serializable {
	...
	public String goPagosNoRealizados(Usuario user) {
		...
		try {
			...
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "No es posible cargar el formulario, Int�ntelo nuevamente!", null);
...
//				Matricula matr = matriculaDAO.get(idMatricula);
    ...
    public void bajaMatricula() {
		...
		if (updtMatriAlum != 0 && idPagoMatri != 0) {
			...
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "OCURRI� UN ERROR EN LA BAJA DE PAGO, "
    ...
    public void bajaMesa() {
    	...
    	try {
					...
					if (idPagoMesa != 0) {
						...
						if (deleteMov != 0) {
							...
							FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
				    				"OCURRI� UN ERROR AL REGISTRAR LA BAJA DEL PAGO DE MESA EN CAJA", null));
						}
					} else {
						FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
			    				"OCURRI� UN ERROR AL REGISTRAR LA BAJA DEL PAGO DE MESA", null));
					}				
				} else {
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
		    				"OCURRI� UN ERROR AL OBTENER EL PAGO DE MESA", null));
				}
    		} else {
    			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
    		...
    		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
    				"OCURRI� UN ERROR AL REGISTRAR LA BAJA DEL PAGO DE MESA. Error: " + e.getMessage(), null));
    	}
    }
    	...
    	try {
    		if(pagosmatricula.getMonto() > 0){  
        		...
        		cajaMov.generarMovimiento(fecha, 1, pagosmatricula.getMonto(), idPagoMatricula, "PagosMatricula", "Pago Matricula", 
        				"Pago de Matr�cula de Curso " + matalumno.getCurso().getNombre() + ", de " + matalumno.getAlumno().getNombreCompleto(), usuario);
    	...
    	try {    		
    		...
    		if (montoImp > 0 && fecha != null) {
    			...
    			cajaMov.generarMovimiento(fecha, 1, montoImp, idPagoMatricula, "PagosMatricula", "Pago Matricula", 
        				"Pago de Deuda de Matr�cula de Curso " + matriculaA.getCurso().getNombre() + ", de " + matriculaA.getAlumno().getNombreCompleto(), usuario);
    ...
    public void pagarMesa() {
    	...
    	try {
    		if (idAlumno != 0 && idMatricula != 0 && idCurso != 0 && idMateria != 0 && idMesa != 0) {
				...
				if (idPagoMesa != 0) {
					...
					if (insertoCaja != 0) {
						...
						FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
		        				"OCURRI� UN ERROR AL REGISTRAR EL PAGO DE LA MESA EN LA CAJA.", null));
					}
				} else {
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
	        				"OCURRI� UN ERROR AL REGISTRAR EL PAGO DE LA MESA.", null));
				}				
    		} else {
    			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
        				"TODOS LOS CAMPOS SON OBLIGATORIOS.", null));
    		...
    		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, 
    				"OCURRI� UN ERROR AL REGISTRAR EL PAGO DE LA MESA. Error: " + e.getMessage(), null));
    	}
    }   
    	...
    	try {
    		...
    		if(parametro.getDiasPrimerVencimiento() != 0 && parametro.getDiasSegundoVencimiento() != 0 
        		...
        		if (updtParam != 0) {
        			...
        			msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR AL REGISTRAR LOS DATOS! INT�NTELO NUEVAMENTE!", null);
        		}
        	} else {
    		...
    		FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "OCURRI� UN ERROR AL REGISTRAR LOS DATOS! ERROR ORIGINAL: " 
    				+ e.getMessage(), null);
    		FacesContext.getCurrentInstance().addMessage(null, msg);
    	}    	
    }
